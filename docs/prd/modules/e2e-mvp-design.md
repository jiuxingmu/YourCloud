# YourCloud 端到端 MVP 设计（KMP + Ktor + PostgreSQL + Web）
...
- 数据库：`PostgreSQL`
- 鉴权：账号密码 + `JWT`
- 交付形态：单体后端服务 + 基于 KMP 共享层的 Web 前端

## 2. 来自 PRD 的关键约束

- 必须保证存储可替换，业务层不得强依赖 COS SDK。
- 一期优先 Android + Web；当前实现先交付 Web 闭环，后端 API 可复用于后续多端。
- 支持私有化方向，关键配置通过环境变量注入。
- 客户端技术必须采用 KMP，禁止绕过共享层直接做孤立 Web 技术栈实现。

## 3. 架构设计

### 3.1 目录结构

- `apps/backend`：Ktor API 服务
- `apps/shared`：KMP 共享模块（网络、领域模型、核心用例）
- `apps/web`：Web 客户端（基于 `apps/shared`，用于一期联调与验证）
...
MVP 阶段先实现 `LocalStorageProvider`，为后续 `CosStorageProvider` 预留扩展点。

### 3.3 客户端分层（KMP）

- `shared/commonMain`：领域模型、仓储接口、用例、网络协议定义
- `shared/androidMain`：Android 平台实现
- `shared/jsMain`：Web 平台实现
- Web UI 层只承载页面和交互，业务流程通过共享用例执行
...
- `M1`：KMP 共享模块 + 后端 + PostgreSQL 脚手架
- `M2`：鉴权（注册/登录/JWT）+ KMP 共享鉴权用例
- `M3`：文件能力（上传/列表/下载）+ KMP 共享文件用例
- `M4`：分享能力（创建/访问/下载）+ KMP 共享分享用例
- `M5`：基础加固（校验、错误码、文档）+ Android/Web 一致性检查
...
- 风险：Web 端为赶进度偏离 KMP 共享架构
  - 对策：业务逻辑统一沉淀到 `shared`，Web 与 Android 仅做平台 UI 与适配
